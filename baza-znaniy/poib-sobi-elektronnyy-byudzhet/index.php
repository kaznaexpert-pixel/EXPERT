...
<title>СОБИ ПОИБ ФК (sobi.cert.roskazna.ru): что это и как получить доступ</title>
...
<meta property="og:title" content="СОБИ ПОИБ ФК (sobi.cert.roskazna.ru): что это и как получить доступ">
...
<meta property="og:image" content="https://kaznaexpert.ru/img/og/baza-znaniy__poib-sobi-elektronnyy-byudzhet-16x9.jpg">
...
<meta name="twitter:title" content="СОБИ ПОИБ ФК (sobi.cert.roskazna.ru): что это и как получить доступ">
...
<meta name="twitter:image" content="https://kaznaexpert.ru/img/og/baza-znaniy__poib-sobi-elektronnyy-byudzhet-16x9.jpg">
...
<div class="layout">
  ...
  <aside class="toc" aria-label="Содержание">
    ...
    <ul class="toc__list" id="tocList">
      ...
      <li><a href="#poib-sobi-roskazna">ПОИБ СОБИ Росказна: адреса и ошибки</a></li>
  ...
  <article class="content" id="main">
    ...
    <h1>СОБИ ПОИБ ФК и ПОИБ СОБИ: что это и как получить доступ к «Электронному бюджету»</h1>
    <div class="byline"><span class="av">ЯМ</span><span>Автор: <a href="/komanda/mihailov-yaroslav/">Ярослав Михайлов</a>, главный эксперт</span><span class="dot">·</span><span>15+ лет в Казначействе</span><span class="dot">·</span><span>12 мин чтения</span><span class="dot">·</span><span>Обновлено 2 сентября 2026</span></div>
    </div>
    ...
    <p data-ctx="giis-root">Где ПОИБ находится в общей архитектуре — среди подсистем и точек входа ГИИС — показано в обзоре <a href="/baza-znaniy/elektronnyy-byudzhet-dlya-ispolnitelya-kontrakta/#giis-sostav">ГИИС «Электронный бюджет»: устройство, подсистемы, адреса</a>.</p>

    <h2 id="formulirovki">ПОИБ СОБИ ФК, СОБИ ФК, sobi.cert.roskazna.ru — это одно и то же</h2>
    <div class="answer reveal"><p>«СОБИ ПОИБ ФК», «ПОИБ СОБИ ФК», «СОБИ ФК», «ПОИБ СОБИ казначейство», «ПОИБ СОБИ Росказна», а также адреса sobi.cert.roskazna.ru и poib.sobi.cert.roskazna.ru — это разные обозначения одной подсистемы безопасности Федерального казначейства для управления полномочиями в ГИИС «Электронный бюджет». Вход выполняется по сертификату электронной подписи через защищённое соединение.</p></div>
    <p>Пользователи ищут этот сервис по-разному: «поиб соби фк» и «соби фк» (ФК — Федеральное казначейство), латиницей по адресу входа «sobi.cert.roskazna.ru: вход» или «poib.sobi.cert.roskazna». Латиница в запросах реальная, потому что адрес набирают прямо из строки браузера. Всё это ведёт в одну точку — защищённый контур СОБИ, где ПОИБ отвечает за назначение полномочий пользователям организации.</p>
    <p>Практический вывод: отдельного «СОБИ ФК» в отрыве от ПОИБ и «Электронного бюджета» не существует — это одна система. Достаточно один раз настроить рабочее место и войти по сертификату, как разобрано ниже.</p>
    ...
    <ul>
      <li><b>Не устанавливается защищённое соединение.</b> Чаще всего не запущен или неверно настроен «Континент TLS-клиент»: проверьте, что клиент установлен, служба работает, а адрес sobi.cert.roskazna.ru добавлен в его конфигурацию.</li>
      <li><b>Сертификат не доверенный или истёк.</b> Установите актуальные корневые сертификаты удостоверяющего центра Федерального казначейства и проверьте срок действия сертификата подписи; просроченный сертификат вход не пропустит.</li>
      <li><b>Страница не открывается совсем.</b> Обычно блокирует прокси или закрытый порт: защищённый канал должен иметь прямой доступ, а браузер — поддерживать криптоалгоритмы ГОСТ.</li>
      <li><b>Ошибка после ввода — «доступ запрещён».</b> Это уже не канал, а полномочия: значит вход прошёл, но нужную роль в ПОИБ ещё не назначил администратор организации.</li>
      <li><b>Неверное время на компьютере.</b> Расхождение системного времени ломает проверку сертификата — синхронизируйте часы.</li>
    </ul>
    <p>Порядок диагностики простой: сначала убедитесь, что «Континент TLS» поднимает канал и страница входа вообще открывается, затем — что сертификат действителен и доверен, и только потом разбирайтесь с полномочиями в ПОИБ. Такой порядок отсекает 90% обращений в поддержку, потому что канал и сертификат — причина большинства сбоев входа.</p>
    ...
    <h2 id="poib-sobi-roskazna">ПОИБ СОБИ Росказна: адреса, что там делают и какие ошибки встречаются</h2>
    <div class="answer reveal"><p>ПОИБ СОБИ Росказна — это три адреса Федерального казначейства: sobi.cert.roskazna.ru, sobi.login.roskazna.ru и sobi.esia.roskazna.ru — под три способа входа: по сертификату, по логину-паролю и через ЕСИА; все три Казначейство велит добавить в надёжные узлы браузера<a class="fn" href="#ref-7" aria-label="Источник 7">[7]</a>. Там ведут реестр организаций, регистрируют пользователей, назначают роли и выдают машиночитаемые доверенности — по Порядку предоставления доступа от 13.03.2023<a class="fn" href="#ref-8" aria-label="Источник 8">[8]</a>.</p></div>

    <h3 id="sobi-chto-delayut">Что живёт на sobi.cert.roskazna.ru: реестр, регистрация, роли, МЧД</h3>
    <p>Система обеспечения безопасности информации ФК (СОБИ) выдаёт доступ ко всем информационным системам, оператор которых — Казначейство, через одну подсистему — ПОИБ. С 2022 года вход в подсистемы «Электронного бюджета» через lk.budget.gov.ru не предоставляется<a class="fn" href="#ref-9" aria-label="Источник 9">[9]</a>. Внутри четыре функции<a class="fn" href="#ref-8" aria-label="Источник 8">[8]</a>:</p>
    <ul>
      <li><b>Реестр организаций</b> — заполняется автоматически из реестров Казначейства, ЕГРЮЛ, ЕГРИП и ЕСИА; при самостоятельной регистрации руководителя по сертификату или через ЕСИА организация создаётся сама.</li>
      <li><b>Регистрация пользователей</b> — автоматически при получении сертификата в УЦ ФК, самостоятельно по сертификату с ИНН и СНИЛС, через подтверждённую учётную запись ЕСИА, либо силами сотрудника с ролью «Регистратор».</li>
    </ul>
    <h3 id="sobi-roli-mchd">Роли и МЧД в ПОИБ СОБИ ФК: кто кому что назначает</h3>
    <p>Полномочия в ПОИБ СОБИ ФК распределяют сами организации — Казначейство лишь проверяет по ЕГРЮЛ право руководителя действовать без доверенности<a class="fn" href="#ref-8" aria-label="Источник 8">[8]</a>.</p>
    <ul>
      <li><b>Роли</b> — руководителю при регистрации автоматически даются роли ГРО (главный регистратор организации) и «Регистратор»; они назначают сотрудникам роли в ПОИБ и в самих системах, а «Визирующий» согласует заявки.</li>
      <li><b>МЧД</b> — руководитель выдаёт сотрудникам машиночитаемые доверенности из классификатора полномочий Минцифры прямо в ПОИБ либо сотрудник подгружает МЧД из реестра ФНС; по истечении срока полномочия снимаются автоматически.</li>
    </ul>

    <h3 id="sobi-vhod-sposoby">Три способа входа в ПОИБ СОБИ ФК</h3>
    <p>Порядок называет три способа входа: по сертификату, через ЕСИА и по паре логин-пароль, где логином служит ИНН или СНИЛС, а пароль задаётся в профиле учётной записи<a class="fn" href="#ref-8" aria-label="Источник 8">[8]</a>. Любое действие с профилем, ролями или МЧД оформляется заявкой и подтверждается квалифицированной подписью — поэтому в сертификате обязательно должны быть ИНН или СНИЛС, совпадающие с профилем. Вход по логину годится, чтобы посмотреть профиль; подписать заявку без сертификата нельзя.</p>

    <h3 id="sobi-sertifikaty">Какие сертификаты и списки отзыва нужны для входа</h3>
    <p>На рабочем месте должны стоять три вещи: личный квалифицированный сертификат пользователя, сертификаты головного удостоверяющего центра Минцифры (ГУЦ) и удостоверяющего центра Федерального казначейства по ГОСТ Р 34.10-2012<a class="fn" href="#ref-7" aria-label="Источник 7">[7]</a>, а также актуальный список аннулированных сертификатов, который Казначейство публикует на crl.roskazna.ru. Где взять файлы и в какие хранилища класть — в разборах <a href="/baza-znaniy/kornevye-sertifikaty-kaznachejstva/">корневых сертификатов казначейства</a> и <a href="/baza-znaniy/sertifikaty-i-spiski-otzyva-uc-kaznachejstva/">сертификатов и списков отзыва УЦ</a>.</p>

    <h3 id="sobi-oshibki-dostupa">Типовые ошибки доступа к ПОИБ СОБИ Росказна и как их снять</h3>
    <table>
      <thead><tr><th>Симптом</th><th>Причина</th><th>Что сделать</th></tr></thead>
      <tbody>
        <tr><td>Нет окна выбора сертификата</td><td>Личный сертификат не установлен в «Личное» или нет привязки к контейнеру; нет плагина КриптоПро ЭЦП Browser Plugin 2.0</td><td>Переустановить сертификат из контейнера, поставить плагин</td></tr>
        <tr><td>sobi.cert.roskazna.ru не открывается, «ошибка TLS»</td><td>Браузер без ГОСТ-шифрования либо адреса не в надёжных узлах</td><td>В Яндекс.Браузере: browser://settings/?search=ГОСТ → включить «Подключаться к сайтам, использующим шифрование по ГОСТ»; inetcpl.cpl → «Безопасность» → «Надёжные сайты» → добавить три адреса sobi.*</td></tr>
        <tr><td>«Сертификат не доверен» или ошибка проверки отзыва</td><td>Нет сертификатов ГУЦ и УЦ ФК либо устарел список аннулированных сертификатов</td><td>Установить цепочку ГУЦ → УЦ ФК, обновить СОС с crl.roskazna.ru</td></tr>
      </tbody>
    </table>
    <h3 id="sobi-oshibki-uchetka">Ошибки учётной записи и регистрации в ПОИБ СОБИ</h3>
    <p>Если страница открывается и сертификат виден, но подсистема не пускает дальше, причина обычно в реквизитах профиля<a class="fn" href="#ref-8" aria-label="Источник 8">[8]</a>.</p>
    <table>
      <thead><tr><th>Симптом</th><th>Причина</th><th>Что сделать</th></tr></thead>
      <tbody>
        <tr><td>Заявка не подписывается</td><td>ИНН или СНИЛС в сертификате не совпадают с профилем учётной записи</td><td>Сверить реквизиты профиля с сертификатом, при расхождении перевыпустить сертификат</td></tr>
        <tr><td>Регистрация через ЕСИА не проходит</td><td>Учётная запись Госуслуг не подтверждена или не привязана к организации, либо организации ещё нет в ПОИБ</td><td>Подтвердить учётную запись и привязку к организации; первым регистрируется руководитель — по сертификату или через ЕСИА</td></tr>
      </tbody>
    </table>
    ...
    <div class="faq">
      ...
      <details><summary>Что такое СОБИ ПОИБ ФК и чем отличается от ПОИБ СОБИ?</summary><p>Ничем: СОБИ ПОИБ ФК, ПОИБ СОБИ ФК и ПОИБ СОБИ Росказна — разный порядок слов для одной подсистемы обеспечения информационной безопасности в составе Системы обеспечения безопасности информации Федерального казначейства. Адрес входа один — sobi.cert.roskazna.ru, через неё выдают роли и МЧД для «Электронного бюджета».</p></details>
      <details><summary>ПОИБ СОБИ Росказна: как зарегистрироваться и войти?</summary><p>Первым регистрируется руководитель — автоматически по реестрам Казначейства, по сертификату с ИНН и СНИЛС или через ЕСИА; сотрудников регистрируют сами по сертификату, через Госуслуги или силами «Регистратора». Войти можно по сертификату, через ЕСИА или по логину (ИНН/СНИЛС) и паролю, но заявки подписываются только квалифицированной подписью.</p></details>
...
<div class="refs">
      ...
      <ol>
        ...
        <li id="ref-7">Раздел «Система обеспечения безопасности информации Федерального казначейства»: требования к рабочему месту, адреса sobi.cert / sobi.login / sobi.esia для надёжных узлов, настройка ГОСТ в Яндекс.Браузере, сертификаты ГУЦ и УЦ ФК, телефон контактного центра. <a href="https://roskazna.gov.ru/gis/sistema-obespecheniya-bezopasnosti-informacii-fk/" target="_blank" rel="noopener">roskazna.gov.ru</a></li>
        <li id="ref-8">Порядок предоставления доступа к ресурсам информационных систем, интегрированных с ПОИБ СОБИ ФК (утверждён Федеральным казначейством 13.03.2023): реестр организаций, регистрация, три способа входа, роли ГРО/Регистратор/Визирующий, МЧД. <a href="https://roskazna.gov.ru/gis/sistema-obespecheniya-bezopasnosti-informacii-fk/poib-sobi-pamyatki-instrukcii/" target="_blank" rel="noopener">roskazna.gov.ru, PDF в разделе «ПОИБ СОБИ (Памятки, инструкции)»</a></li>
        <li id="ref-9">Краткая инструкция по доступу в личный кабинет ГИИС «Электронный бюджет» через СОБИ (Федеральное казначейство): доступ с 2022 года только через ПОИБ СОБИ, точка входа eb.cert.roskazna.ru. <a href="https://roskazna.gov.ru/uploads/migrate/roskaznagovru/documents/gis/sistema-obespecheniya-bezopasnosti-informacii-fk/Instruktsiya-dostupa-k-GIIS-Elektronnyy-byudzhet-cherez-SOBI.DOCX" target="_blank" rel="noopener">roskazna.gov.ru, DOCX</a></li>
      </ol>
      <p class="refs__note">Сведения приведены по состоянию на 2 сентября 2026 года. Актуальные адреса входа, версии программного обеспечения и инструкции смотрите на официальном сайте Федерального казначейства roskazna.gov.ru.</p>
    </div>
